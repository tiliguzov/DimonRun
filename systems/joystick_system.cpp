...
void JoystickSystem::Update() {
  for (const auto& entity : entities_) {
    auto& motion_comp = coordinator_->GetComponent<core::MovementComponent>(entity);
    ...
    if (static_cast<int>(position_comp.position.x()) % core::kTextureSize == 0 &&
        static_cast<int>(position_comp.position.y()) % core::kTextureSize == 0) {
